...
var soundcloudRequest = function(url, done) {
	...
	if (e) {
		...
		var item = {
		    ...
		    stream_url: streamUrl(data.tracks[i].stream_url),
		...
		stream_url: streamUrl(data.stream_url),
		title: data.title,
...
module.exports = function(url, uri, done) {
    ...
    var excludedP2 = ['groups', 'dropbox', 'following', 'followers', 'likes', 'comments', 'tracks', 'sets', 'spotlight'];
